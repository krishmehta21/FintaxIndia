...
def admin_list_blog_posts():
    response = supabase.table("blog_posts").select("id, title, slug, excerpt, cover_image_url, category, status, published_at, created_at, updated_at, author").order("created_at", desc=True).execute()
    return response.data
